...
void test_ncodec__ncodec_open(void** state)
{
    ...
    assert_null(sv->vtable.codec(sv, 0));
    assert_null(sv->vtable.codec(sv, 1));
    assert_non_null(sv->vtable.codec(sv, 2));
    assert_null(sv->ncodec[0]);
    assert_null(sv->ncodec[1]);
    assert_non_null(sv->ncodec[2]);
    assert_ptr_equal(sv->vtable.codec(sv, 2), sv->ncodec[2]);
}
...
void test_ncodec__network_stream(void** state)
{
    ...
    sv->vtable.release(sv, 2);
    sv->vtable.append(sv, 2, buffer, len);
    NCodecCanMessage msg = {};
    len = ncodec_read(nc, &msg);
