 ...
 *  selector : SchemaObjectSelector (pointer to)
 ...
 *  handler : SchemaMatchHandler (function)
 ...
 *  #include <dse/modelc/schema.h>
 *
 *  int match_handler(ModelInstanceSpec* model_instance, SchemaObject* object)
 *  {
 *      uint32_t index = 0;
 *      SchemaSignalObject* so;
 *      do {
 *          so = schema_object_enumerator(model_instance, object,
 *                  "spec/signals", &index, schema_signal_object_generator);
 *          if (so == NULL) break;
 *          if (so->signal) {
 ...
 *  }
 *
 *  void object_search(ModelInstanceSpec* model_instance, const char*
 * channel_name)
 *  {
 ...
 *      if (selector) {
 *          schema_object_search(model_instance, selector, match_handler);
 *      }
 ...
 *
 *  Build a channel selector object based on the provided Model Instance and
 ...
 *  Note
 *  ----
 *  A channel selector will not match on metadata/name.
 *
 *  Parameters
 *  ----------
 *  model_instance : ModelInstanceSpec
 *      The Model Instance, which holds references to the various schema objects
 *      which will be searched.
 *  channel : ChannelSpec (pointer to)
 ...
 *  Returns
 *  -------
 *      SchemaObjectSelector (pointer to) : The complete selector object.
 *      NULL : A selector object could not be created, which does not indicate
 *          and error condition.
 */
 ...
 *
 *  labels[] may be dynamically allocated and would need to be free'ed.
 */
 ...
 *      The Model Instance, which holds references to the various schema objects
 *      which will be searched.
 *  object : SchemaObject (pointer to)
 *      The schema object to enumerate from.
 *  path : const char (pointer to)
 *      Enumerate objects located at this path.
 *  index : uint32_t (pointer to)
 ...
 *  Returns
 *  -------
 *      void* : The generated schema object.
 ...
 *      The Model Instance, which holds references to the various schema objects
 *      which will be searched.
 *  data : void (pointer to)
 *      The YAML node to generate from.
 *
 *  Returns
 *  -------
 *      void* : The generated schema signal object.
 *      NULL : If the object cannot be generated.
 */
