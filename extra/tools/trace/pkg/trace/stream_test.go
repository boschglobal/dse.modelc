...
func (v TestVisitor) Handle(f Flatbuffer) {
	f.Accept(v)
}

func TestStreamIteratorVisitor(t *testing.T) {
	v := TestVisitor{monitor: &[]Flatbuffer{}}
	trace := Stream{stack: []Flatbuffer{NotifyMsg{}, ChannelMsg{}}}
	for m := range trace.Messages() {
		m.Accept(v)
	}

	assert.Len(t, *v.monitor, 2)
	assert.IsType(t, NotifyMsg{}, (*v.monitor)[0])
	assert.IsType(t, ChannelMsg{}, (*v.monitor)[1])
}
...
func TestSimBusStream(t *testing.T) {
	...
	assert.Nil(t, err)
	assert.Len(t, *v.monitor, 24)
	assert.IsType(t, ChannelMsg{}, (*v.monitor)[0])
	assert.IsType(t, ChannelMsg{}, (*v.monitor)[1])
	assert.IsType(t, NotifyMsg{}, (*v.monitor)[12])
	assert.IsType(t, NotifyMsg{}, (*v.monitor)[21])
	assert.IsType(t, ChannelMsg{}, (*v.monitor)[22])
	assert.IsType(t, ChannelMsg{}, (*v.monitor)[23])
}
